...
function MoviesCard({
  ...
  return (
    ...
    <section className="moviesCard">
      ...
      <p className="moviesCard__duration">{transferToHouse(movie.duration)}</p>
      </div>
      {!savedMoviesPage ? (
      <button onClick={saved ? handleClickDelete : handleClickSave}
      className={
        saved
    ...
    ></button>
      ...
      src={
        ...
        ? `${movie.image}`
        : `https://api.nomoreparties.co${movie.image.url}`} alt="Обложка фильма" />
    </section>
    </>
  );
}
